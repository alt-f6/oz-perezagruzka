...
import { BUSINESS_TIMEZONE, zonedWallClockToUtc } from "@/shared/lib/timezone";
 ...
 * interpreting the result as the given timezone and returning the matching UTC
 * instant. Using the explicit timezone here — instead of the ambient
 ...
 */
function atTime(day: Date, time: string, timeZone: string): Date {
...
export function expandOccurrences(
  ...
  if (values.recurrence === "NONE") {
    return [
      {
        scheduledAt: atTime(start, values.time, timeZone),
  ...
  while (cursor <= endDate && occurrences.length < MAX_RECURRING_SESSIONS) {
    if (allowedDays.has(cursor.getDay())) {
      ...
      occurrences.push({
        scheduledAt: atTime(cursor, time, timeZone),
        durationMinutes,
      });
    }
