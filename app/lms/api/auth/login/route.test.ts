...
function loginRequest(body: object) {
  return new NextRequest("http://localhost/lms/api/auth/login", {
    method: "POST",
...
describe("POST /lms/api/auth/login", () => {
  beforeEach(() => {
  ...
  it("clears login attempts by email (not ip+email) on success", async () => {
    authenticateMock.mockResolvedValue({ id: "u1", role: "STUDENT", email: "a@x.com" });
    const { POST } = await import("./route");
