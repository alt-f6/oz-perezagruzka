...
const logger = createLogger("landing.readiness-map.share-section");
  ...
  const handleNativeShare = async () => {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        ...
        logger.warn("Native share failed or dismissed", { err });
      }
    } else {
      handleCopy();
    }
  ...
  return (
    <div className="mt-8 border-t border-ink-100 pt-8 no-print">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        >
          <svg className="h-5 w-5 text-[var(--color-telegram-share)] fill-current" viewBox="0 0 24 24">
        ...
        >
          <svg className="h-5 w-5 text-[var(--color-whatsapp)] fill-current" viewBox="0 0 24 24">
