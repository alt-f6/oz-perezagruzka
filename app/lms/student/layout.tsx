...
export default async function StudentLayout({ children }: { children: React.ReactNode }) {
  // MANAGER (and ADMIN via bypass) are let through so the admin
  ...
  // TEACHER previews in its own workspace (/teacher/lessons/[id]) instead,
  // where nothing writes student progress.
  const user = await requireRoleForPage(["STUDENT", "MANAGER"], {
    adminBypass: true,
    loginPath: "/login",
