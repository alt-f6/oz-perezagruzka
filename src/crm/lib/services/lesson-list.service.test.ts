...
describe("listLessons", () => {
  ...
  it("requests limit+1 rows ordered by scheduledAt desc and returns nextCursor from the extra row", async () => {
    findManyMock.mockResolvedValue([
    ...
    expect(findManyMock).toHaveBeenCalledWith(
      expect.objectContaining({ orderBy: { scheduledAt: "desc" }, take: 2 }),
    );
