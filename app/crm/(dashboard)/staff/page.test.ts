...
vi.mock("@/shared/lib/auth", () => ({
  ...
  ALL_ROLES: ["ADMIN", "MANAGER", "TEACHER", "STUDENT", "PARENT"],
}));
...
describe("CRM staff page guard", () => {
  it("redirects a MANAGER (not ADMIN) to /access-denied", async () => {
    ...
    await expect(StaffPage()).rejects.toThrow("NEXT_REDIRECT:/access-denied");
  });

  it("redirects an unauthenticated visitor to /admin/login", async () => {
    getSessionUserMock.mockResolvedValue(null);
    await expect(StaffPage()).rejects.toThrow("NEXT_REDIRECT:/admin/login");
  });
});
